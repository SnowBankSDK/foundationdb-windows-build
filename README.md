...
lines, and its releases also carry the macOS arm64 client built separately.

## Releases

Each [release](https://github.com/SnowBankSDK/foundationdb-windows-build/releases) is named after the
upstream tag it was built from and carries `fdb_c.dll`, `fdbcli.exe`, `libfdb_c.dylib`, `fdbcli` (macOS)
and a `.sha256` file per binary. The `FoundationDB.Client.Native` NuGet package downloads these files by
URL and verifies the checksums.
